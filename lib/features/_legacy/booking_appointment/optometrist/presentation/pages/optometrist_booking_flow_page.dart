...
import 'package:m2health/features/_legacy/booking_appointment/optometrist/presentation/bloc/optometrist_booking_flow_bloc.dart';
import 'package:m2health/service_locator.dart';
import 'package:m2health/route/appointment_routes.dart';

class OptometristBookingFlowPage extends StatefulWidget {
  const OptometristBookingFlowPage({super.key});

  @override
  State<OptometristBookingFlowPage> createState() =>
      _OptometristBookingFlowPageState();
}

class _OptometristBookingFlowPageState
    extends State<OptometristBookingFlowPage> {
  late final OptometristBookingFlowBloc _flowBloc;
  ...
  void initState() {
    super.initState();
    _flowBloc = OptometristBookingFlowBloc(createAppointment: sl());
  ...
  Widget build(BuildContext context) {
    return MultiBlocProvider(
      ...
      child: PopScope(
        ...
        child: BlocConsumer<OptometristBookingFlowBloc,
            OptometristBookingFlowState>(
          ...
          listener: (context, state) {
            if (state.createdAppointment != null) {
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(
                  content: Text('Optometry appointment booked successfully!'),
  ...
  List<Page<Object?>> _buildPages(OptometristBookingFlowState state) => [
        MaterialPage(
