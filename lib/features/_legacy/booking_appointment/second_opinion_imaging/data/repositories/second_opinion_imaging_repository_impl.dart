...
import 'package:m2health/features/_legacy/booking_appointment/second_opinion_imaging/data/datasources/second_opinion_imaging_remote_datasource.dart';
import 'package:m2health/features/_legacy/booking_appointment/second_opinion_imaging/domain/repositories/second_opinion_imaging_repository.dart';

class SecondOpinionImagingRepositoryImpl
    extends SecondOpinionImagingRepository {
  ...
  Future<Either<Failure, AppointmentEntity>> createAppointment(
    ...
    try {
      ...
      formData.fields
          .add(MapEntry('provider_id', params.providerId.toString()));
      formData.fields.add(
          MapEntry('start_datetime', params.startDatetime.toIso8601String()));
      formData.fields.add(MapEntry('summary', params.summary));

      formData.fields
          .add(MapEntry('request_data[service_type]', params.serviceType));
      formData.fields
          .add(MapEntry('request_data[disease_name]', params.diseaseName));
      if (params.diseaseHistory != null) {
        formData.fields.add(
            MapEntry('request_data[disease_history]', params.diseaseHistory!));
      }
      if (params.biomarker != null) {
        formData.fields
            .add(MapEntry('request_data[biomarker]', params.biomarker!));
      }
      ...
      for (int i = 0; i < params.images.length; i++) {
        ...
        if (img.imageType != null) {
          formData.fields.add(
              MapEntry('request_data[images][$i][image_type]', img.imageType!));
        }
      ...
      final response =
          await appointmentService.createAppointmentMultipart(formData);
