...
import 'package:m2health/features/_legacy/booking_appointment/psychologist/presentation/bloc/psychologist_booking_flow_bloc.dart';
import 'package:m2health/service_locator.dart';
import 'package:m2health/route/appointment_routes.dart';

class PsychologistBookingFlowPage extends StatefulWidget {
  const PsychologistBookingFlowPage({super.key});

  @override
  State<PsychologistBookingFlowPage> createState() =>
      _PsychologistBookingFlowPageState();
}

class _PsychologistBookingFlowPageState
    extends State<PsychologistBookingFlowPage> {
  late final PsychologistBookingFlowBloc _flowBloc;
  ...
  void initState() {
    super.initState();
    _flowBloc = PsychologistBookingFlowBloc(createAppointment: sl());
  ...
  Widget build(BuildContext context) {
    return MultiBlocProvider(
      providers: [
        BlocProvider.value(value: _flowBloc),
      ...
      child: PopScope(
        ...
        child: BlocConsumer<PsychologistBookingFlowBloc,
            PsychologistBookingFlowState>(
          ...
          listener: (context, state) {
            if (state.createdAppointment != null) {
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(
                  content: Text('Psychology appointment booked successfully!'),
  ...
  List<Page<Object?>> _buildPages(PsychologistBookingFlowState state) => [
        MaterialPage(
