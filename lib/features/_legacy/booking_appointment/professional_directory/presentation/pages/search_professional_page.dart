...
class _SearchProfessionalPageState extends State<SearchProfessionalPage> {
  ...
  Future<void> _onAddressesStateChanged(SavedAddressesState state) async {
    ...
    if (state.addresses.isNotEmpty) {
      ...
      setState(() {
        ...
        _location = VisitLocation.fromAddress(defaultAddress);
      });
    ...
    // Nothing saved. Browse from where the device already is, without
    // prompting and without writing anything to the address book.
    final current = await CurrentLocationService().resolveIfGranted();
    if (!mounted || current == null) return;
    setState(() => _location = current);
    _fetchProfessionals(query: _searchController.text);
  }
  ...
  void _onAddressSelected(Address address) {
    ...
    setState(() {
      ...
      _location = VisitLocation.fromAddress(address);
    });
  ...
  Future<void> _showAddressPicker() async {
    final state = _addressesCubit.state;
    final picked = await showVisitLocationPicker(
      context,
      addresses: state is SavedAddressesLoaded ? state.addresses : const [],
      selected: _location,
    );
    if (picked == null || !mounted) return;

    if (picked.isSaved) {
      final saved = (state as SavedAddressesLoaded)
          .addresses
          .firstWhere((a) => a.id == picked.addressId);
      _onAddressSelected(saved);
      return;
    }

    // This flow hands a saved Address downstream, so a deliberate one-off pick
    // is persisted here rather than at submit.
